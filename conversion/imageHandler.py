...
class ImageHandler:
    ...
    def setImageDate(self, event):
        ...
        for i in range(len(self.fileData)):
            if self.fileData[i]["date"] == self.main.date_entry.get():
                messagebox.showinfo("Error", "Date already assigned.", parent=self.main.root)
                return
    ...
    def exportImages(self):
        ...
        if(len(self.fileData) == 0):
            messagebox.showinfo("Error", "No images have been loaded.", parent=self.main.root)
            return
        ...
        messagebox.showinfo("Export Complete", "Images were successfully exported.", parent=self.main.root)
